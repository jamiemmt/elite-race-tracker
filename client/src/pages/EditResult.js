...
const EditResult = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch result, athletes, and races in parallel
        const [resultRes, athletesRes, racesRes] = await Promise.all([
        ...
        setFormData({
          athlete: result.athlete._id,
          race: result.race._id,
          finishTime: result.finishTime.toString(),
  ...
  const onSubmit = async e => {
    ...
    try {
      ...
      await axios.put(`/api/results/${id}`, resultData);
      
      setLoading(false);
      navigate('/results');
    } catch (err) {
      setError(err.response?.data?.msg || 'Failed to update result. Please try again.');
      setLoading(false);
  ...
  return (
    <Container>
      ...
      <h1 className="page-header">Edit Result</h1>
            ...
            <div className="d-flex justify-content-end">
              ...
              >
                {loading ? (
                  ...
                  <>
                    <FaSave className="me-2" /> Update Result
                  </>
                )}
              </Button>
            </div>
